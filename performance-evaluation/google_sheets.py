...
def get_template_sheets(google_credentials):
    template_auxiliar_sheet = google_credentials.open_by_key(
        TEMPLATE_AUXILIAR_FILE_KEY)
    template_talent_sheet = google_credentials.open_by_key(
        TEMPLATE_TALENT_FILE_KEY)
...
def get_destiny_sheet(google_credentials, key):
    while True:
        try:
            ...
            print('Se va a actualizar el documento con el nombre: \'' +
                  destiny_sheet.title + '\'')
...
def get_auto_evaluation_sheet(google_credentials):
    while True:
        ...
        try:
            ...
            print('Se va a utilizar el documento de auto evaluación con el nombre: \'' +
                  destiny_sheet.title + '\'')
            print('Es correcto? Presione \'s/n\'.')
            if readchar.readchar() == 's':
                print('Abriendo documento para comenzar a trabajar...\n')
...
def get_manager_evaluation_sheet(google_credentials):
    while True:
        ...
        try:
            destiny_sheet = google_credentials.open_by_key(destiny_file_key)
            print('Se va a utilizar el documento de evaluación con el nombre: \'' +
                  destiny_sheet.title + '\'')
            print('Es correcto? Presione \'s/n\'.')
            if readchar.readchar() == 's':
                print('Abriendo documento para comenzar a trabajar...\n')
